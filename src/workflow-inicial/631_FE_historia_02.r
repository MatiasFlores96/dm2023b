...
dataset.task = makeClassifTask(data = dataset, target = "clase_ternaria")
estimateTimeTuneRanger(dataset.task)
res = tuneRanger(dataset.task, measure = list(multiclass.brier), num.trees = 1000, 
...
PARAM$RandomForest$min.node.size <- res$recommended.pars$min.node.size
PARAM$RandomForest$mtry <- res$recommended.pars$mtry
PARAM$RandomForest$semilla <- 400067 # cambiar por la propia semilla
...
PARAM$CanaritosAsesinos$semilla <- 400067
