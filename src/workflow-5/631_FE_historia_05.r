...
PARAM <- list()
PARAM$experimento <- "FE63105"

PARAM$exp_input <- "DR62105"
...
#PARAM$RandomForest$splitrule <- "gini"
PARAM$RandomForest$importance <- "impurity"
...
AgregaVarRandomForest <- function(
    num.trees, max.depth,
    min.node.size, mtry, semilla, importance) {
  gc()
...
if (PARAM$RandomForest$run) {
  ...
  AgregaVarRandomForest(
    ...
    importance = PARAM$RandomForest$importance
  )
